...
#include "Config.h"
    ...
    float reductionPercentage = 0.2f; // 20% reduction for the following allied races.
    ...
    if (player->getRace() == RACE_NIGHTBORNE || 
		...
		player->getRace() == RACE_ZANDALARI_TROLL  || 
		player->getRace() == RACE_KUL_TIRAN  ||		
   		player->getRace() == RACE_DARK_IRON_DWARF ||
		player->getRace() == RACE_VULPERA ||
		player->getRace() == RACE_MECHAGNOME ||  
