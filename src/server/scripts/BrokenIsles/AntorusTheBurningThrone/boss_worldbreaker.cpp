...
enum Spells
{
    ...
    //Cannons
    //122778 cast    
    //SPELL_ANNIHILATION = 244790,//244758,//244761,  244790
    //122773 cast
...
struct boss_garothi_worldbreaker : public BossAI
{
    ...
    void JustDied(Unit* /*killer*/) override
    {
      ...
      /*  if (instance)
        ...
        CleanupEncounter();
        instance->SetCheckPointPos(Position(-3299.84f, 9782.797f, -63.40741f, 1.480368f));
    ...
    void SpellHit(Unit* /*caster*/, SpellInfo const* spell) override
    {
        ...
        {
            if (!(instance->GetCreature(DATA_DECIMATOR)) && !(instance->GetCreature(DATA_ANNIHILATOR)))
                RespawnCreatures();
            ...
            {
                if (!decimator->HasAura(SPELL_RIDE_SPELL))
                    decimator->CastSpell(me, SPELL_RIDE_SPELL, true);
            ...
            {
                if (!annihilator->HasAura(SPELL_RIDE_SPELL))
                    annihilator->CastSpell(me, SPELL_RIDE_SPELL, true);
    ...
    void RespawnCreatures()
    {
        ...
        GetCreatureListWithEntryInGrid(creatures, me, NPC_ANNIHILATOR, 1000.0f);

        if (creatures.empty())
        {
            if (Creature* npc = me->SummonCreature(NPC_ANNIHILATOR, pos1, TEMPSUMMON_MANUAL_DESPAWN, WEEK, 5430, false))
        ...
        else
        {
            for (std::list<Creature*>::iterator iter = creatures.begin(); iter != creatures.end(); ++iter)
            {
                if ((*iter)->isDead())
                    (*iter)->Respawn();
                if (!(*iter)->HasAura(SPELL_RIDE_SPELL))
                    (*iter)->CastSpell(me, SPELL_RIDE_SPELL, true);
            }
        }

        GetCreatureListWithEntryInGrid(creatures, me, NPC_DECIMATOR, 1000.0f);
...
private:
    ...
    void HideCannons()
    {
        ...
        {
            instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, decimator);
            decimator->AddUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
        }

        if (Creature* annihilator = instance->GetCreature(DATA_ANNIHILATOR))
        {
            instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, annihilator);
            annihilator->AddUnitFlag(UNIT_FLAG_NOT_SELECTABLE );
        }
    }
};
// AT: 15496
...
class spell_garothi_annihilation_selector : public SpellScript
{
    ...
    {
        targets.remove_if(IsNotEntryButPlayer(GetCaster(), NPC_GAROTHI_WORLDBREAKER));
        targets.remove_if(IsNotEntryButPlayer(GetCaster(), NPC_ANNIHILATOR));
        targets.remove_if(IsNotEntryButPlayer(GetCaster(), NPC_DECIMATOR));
    }
    ...
    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_garothi_annihilation_selector::CheckTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        OnEffectHitTarget += SpellEffectFn(spell_garothi_annihilation_selector::HandleHit, EFFECT_0, SPELL_EFFECT_DUMMY);
    }
};

class spell_garothi_annihilation_triggered : public AuraScript
{
    PrepareAuraScript(spell_garothi_annihilation_triggered);
    ...
    void AfterRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        if (GetTarget()->HasAura(SPELL_ANNIHILATION_WARNING))
            target->CastSpell(target, SPELL_ANNIHILATION_DAMAGE_UNSPLITTED, true);
    }

    void Register() override
    {
        AfterEffectRemove += AuraEffectRemoveFn(spell_garothi_annihilation_triggered::AfterRemove, EFFECT_0, SPELL_AURA_AREA_TRIGGER, AURA_EFFECT_HANDLE_REAL);
    }
};
