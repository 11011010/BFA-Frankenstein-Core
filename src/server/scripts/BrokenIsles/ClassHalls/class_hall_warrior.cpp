...
struct npc_sergeant_dalton_108961 : public ScriptedAI
{
    ...
    void sQuestAccept(Player* player, Quest const* quest) override
    {
        ...
        {
            if (player->IsInAlliance() && player->GetAreaId() == 7502 && player->getClass() == CLASS_WARRIOR)
...
struct npc_class_hall_warrior_aerylia : public ScriptedAI
{
    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId) override
    {
        ...
        case 0:
            ...
            if (player->HasQuest(QUEST_THE_HUNTER_OF_HEROES))
            {
                player->KilledMonsterCredit(98032);
                player->TeleportTo(1511, Position(3305.7f, 1385.04f, 186.3f, 0.715585f));
                return;
            }
            break;
        case 2:
            spellId = SPELL_TELE_DALARAN;
            if (player->HasQuest(QUEST_THE_SWORD_OF_KINGS))
            {
...
struct npc_danica_the_reclaimer : public ScriptedAI
{
    ...
    void sQuestAccept(Player* player, Quest const* /*quest*/) override
    {
        ...
        if (!summon)
            return;
    //    summon->SetGuidValue(UNIT_FIELD_DEMON_CREATOR, player->GetGUID());
    }
private:
...
class warrior_playerchoice : public PlayerScript
{
public:
    warrior_playerchoice() : PlayerScript("warrior_playerchoice") { }

  //  void OnQuestStatusChange(Player* player, uint32 /*questId*/)
  //  {
      //  if (player->getClass() == CLASS_WARRIOR || player->getLevel() >= 98)
         //   OnCheckPhase(player);
 //   }

    void OnPlayerChoiceResponse(Player* player, uint32 choiceId, uint32 responseId)
    {
        if (choiceId != PLAYER_CHOICE_WARRIOR_SELECTION)
            return;

        if (player->HasQuest(QUEST_WEAPONS_OF_LEGEND))
            player->KilledMonsterCredit(100583);
        if (player->HasQuest(QUEST_ONE_MORE_LEGEND))
            player->KilledMonsterCredit(113830);
        if (player->HasQuest(QUEST_MORE_WEAPONS_OF_LEGEND))
            player->KilledMonsterCredit(111919);
        switch (responseId)
        {
        case ACTION_RESPONSEID_1:
            //if (const Quest* quest_1 = sObjectMgr->GetQuestTemplate(QUEST_ARMS_CHOSEN))
            //    player->AddQuest(quest_1, nullptr);
            if (player->GetQuestStatus(QUEST_THE_SWORD_OF_KINGS) == QUEST_STATUS_NONE)
                if (const Quest* quest_1 = sObjectMgr->GetQuestTemplate(QUEST_THE_SWORD_OF_KINGS))
                    player->AddQuest(quest_1, nullptr);
            //QUEST_THE_SWORD_OF_KINGS
            break;
        case ACTION_RESPONSEID_2:
            //if (const Quest* quest_1 = sObjectMgr->GetQuestTemplate(QUEST_FURY_CHOSEN))
            //    player->AddQuest(quest_1, nullptr);
            if (player->GetQuestStatus(QUEST_THE_HUNTER_OF_HEROES) == QUEST_STATUS_NONE)
                if (const Quest* quest_2 = sObjectMgr->GetQuestTemplate(QUEST_THE_HUNTER_OF_HEROES))
                    player->AddQuest(quest_2, nullptr);
            break;
        case ACTION_RESPONSEID_3:
            //if (const Quest* quest_1 = sObjectMgr->GetQuestTemplate(QUEST_PROT_CHOSEN))
            //    player->AddQuest(quest_1, nullptr);
            if (player->GetQuestStatus(QUEST_LEGACY_OF_THE_ICEBREAKER) == QUEST_STATUS_NONE)
                if (const Quest* quest_3 = sObjectMgr->GetQuestTemplate(QUEST_LEGACY_OF_THE_ICEBREAKER))
...
struct npc_high_overlord_saurfang_93773 : public ScriptedAI
{
    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
    {
        ...
        {
            player->KilledMonsterCredit(93773);
            me->Say(114668);
        }
        if (player->HasQuest(QUEST_H_RETURN_TO_THE_BROKEN_SHORE) || player->HasQuest(QUEST_A_RETURN_TO_THE_BROKEN_SHORE))
        {
            player->SummonCreature(100519, me->GetNearPosition(5.0f, me->GetFollowAngle()), TEMPSUMMON_MANUAL_DESPAWN, 0, 0, true);
            if (Creature* target = player->GetSummonedCreatureByEntry(100519))
                target->AddNpcFlag(UNIT_NPC_FLAG_SPELLCLICK);
        }
    }
private:
};
...
struct npc_makka_100519 : public ScriptedAI
{
    ...
    void OnSpellClick(Unit* clicker, bool& /*result*/)
    {
        ...
        {
            if (player->GetQuestStatus(QUEST_H_RETURN_TO_THE_BROKEN_SHORE) == QUEST_STATUS_INCOMPLETE || player->GetQuestStatus(QUEST_A_RETURN_TO_THE_BROKEN_SHORE) == QUEST_STATUS_INCOMPLETE)
            {
                player->AddDelayedTeleport(2000, 1500, Position(-1873.97f, -1207.44f, 0.14f, 3.45f));
                player->KilledMonsterCredit(me->GetEntry());
                player->KilledMonsterCredit(96678);
                me->DespawnOrUnsummon(5000);
            }
        }
    }
...
struct npc_hruthnir_96660 : public ScriptedAI
{
    ...
    void sGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
    {
        ...
        if (player->HasQuest(QUEST_LEGACY_OF_THE_ICEBREAKER))
            player->TeleportTo(1495, Position(4916.261f, -92.8875f, 66.2577f, 2.624518f));
    }
};

struct npc_eye_of_odyn_97389 : public ScriptedAI
{
    npc_eye_of_odyn_97389(Creature* creature) : ScriptedAI(creature) {  }

    void Reset() override
    {
        me->AddUnitState(UNIT_STATE_CANNOT_TURN);
        me->SetDisplayId(69052);//69052 64074
    }
};
...
void AddSC_class_hall_warrior()
{
    ...
    new warrior_playerchoice();
    RegisterCreatureAI(npc_hruthnir_96660);
    RegisterCreatureAI(npc_eye_of_odyn_97389);
