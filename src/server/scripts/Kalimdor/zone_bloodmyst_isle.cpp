                ...
                if (_moveTimer <= diff)
                {
                    switch (_phase)
                    {
                        ...
                        case PHASE_PLANT_FIRST_STAND: // plant first explosives stage 1 stand
                            ...
                            _moveTimer = 0.5f * AsUnderlyingType(IN_MILLISECONDS);
                            _phase = PHASE_PLANT_FIRST_WORK;
                            break;
                        case PHASE_PLANT_FIRST_WORK: // plant first explosives stage 2 work
                            Talk(SAY_LEGOSO_4);
                            _moveTimer = 17.5f * AsUnderlyingType(IN_MILLISECONDS);
                        ...
                        case PHASE_PLANT_FIRST_FINISH: // plant first explosives finish
                            ...
                            Talk(SAY_LEGOSO_5);
                            _moveTimer = 1.5f * AsUnderlyingType(IN_MILLISECONDS);
                            _phase = PHASE_CONTINUE;
                            break;
        ...
        void WaypointReached(uint32 waypointId) override
        {
            ...
            switch (waypointId)
            {
                case WP_START:
                    ...
                    me->SetFacingToObject(player);
                    Talk(SAY_LEGOSO_1);
                    _moveTimer = 2.5f * AsUnderlyingType(IN_MILLISECONDS);
                    _phase = PHASE_CONTINUE;
                    break;
                ...
                case WP_EXPLOSIVES_SECOND_BATTLEROAR:
                    SetEscortPaused(true);
                    _moveTimer = 0.2f * AsUnderlyingType(IN_MILLISECONDS);
