...
struct bfa_boss_queen_azshara : public BossAI
{
    ...
    public:
        ...
        void Intermission()
        {
            ...
            for (uint8 i = 0; i < 4; ++i)
            {
                Position random = me->GetRandomPoint(centerPosAzshara, 15.0f);
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                {
                    me->SummonCreature(NPC_OVERZEALOUS_HULK, centerPosAzshara);
                    events.ScheduleEvent(EVENT_SUMMON_HULK, TIMER_SUMMON_HULK);
                    break;
                }
                case EVENT_CHANGE_PHASE2:
                {
                    ...
                    if (Creature* ward = me->SummonCreature(NPC_WARD, centerPosAzshara))
                ...
                {
                    me->SummonCreature(NPC_LOYAL_MYRMIDON, centerPosAzshara);
                ...
                case EVENT_NETHER_PORTAL:
                {
                    ...
                    Position portalPos = me->GetRandomPoint(centerPosAzshara, 50.0f);
                ...
                case EVENT_PIERCING_GAZE:
                {
                    ...
                    me->CastSpell(centerPosAzshara, SPELL_PIERCING_GAZE_AT, true);
                    events.ScheduleEvent(EVENT_PIERCING_GAZE, TIMER_PIERCING_GAZE);
                    break;
                }
...
public:
    ...
    class bfa_spell_beckon_cast_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Creature* azshara = caster->ToCreature())
            {
                if (CAST_AI(bfa_boss_queen_azshara, azshara->AI())->_beckonphase2 == true)
                {
                    ...
                    target->MovePosition(centerPosAzshara, 10.0f, 15.0f);
            }
        }
        ...
        void HandlePeriodic(AuraEffect const* aureff)
        {
            ...
            if (!phase_changed)
            {
                if (target->GetDistance(centerPosAzshara) <= 2.0f)
                {
                    this->Remove();
    ...
    void CheckPlayers()
        {
            ...
            if (_normal)
            {
                ...
                {
                    me->CastSpell(centerPosAzshara, SPELL_PRESSURE_SURGE, true);
                    if (Creature* azshara = GetAzshara())
                    {
                        if (!_surge)
                        {
...
private:
    ...
    void IsSummonedBy(Unit* summoner) override
    {
        ...
        this->bounceCount = 0;
        me->AddAura(SPELL_LIGHTNING_ORB_VISUAL);
