        ...
        bool HandleDarkBeyond()
        {
            ...
            if (!playerList.empty())
            {
                for (auto player : playerList)
                {
                    ...
                    {
                        if (Aura* dark = player->AddAura(SPELL_DARK_BEYOND_AURA))
                        {
    ...
    class bfa_spell_delirium_realm_cast_SpellScript : public SpellScript
    {
        ...
        void HandleDummy()
        {
            ...
            for (uint8 i = 0; i < 3; ++i)
            {
                ...
                caster->CastSpell(x, y, z, SPELL_CAUSTIC_DELIRIUM_AT, true);
            }
        }
                ...
                if (timer <= diff)
                {
                    ...
                    if (!playerList.empty())
                    {
                        for (auto player : playerList)
                        {
                            ...
                            {
                                if (Aura* pool = player->AddAura(SPELL_CAUSTIC_DELIRIUM_PERIODIC))
                                {
                ...
                if (timer <= diff)
                {
                    ...
                    if (!playerList.empty())
                    {
                        for (auto player : playerList)
                        {
                            ...
                            {
                                if (Aura* pool = player->AddAura(SPELL_NIGHTMARE_POOL_AURA))
                                {
                                    pool->SetMaxDuration(2000);
                                    pool->SetDuration(2000);
                                }
