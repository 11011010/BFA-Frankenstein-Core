    ...
    public:
        ...
        struct boss_warp_splinterAI : public BossAI
        {
            ...
            void SummonTreants()
            {
                for (uint8 i = 0; i < 6; ++i)
                {
                    float angle = (float(M_PI) / 3) * float(i);

                    float X = Treant_Spawn_Pos_X + AsUnderlyingType(TREANT_SPAWN_DIST) * std::cos(angle);
                    float Y = Treant_Spawn_Pos_Y + AsUnderlyingType(TREANT_SPAWN_DIST) * std::sin(angle);
