...
void TransportMgr::GeneratePath(GameObjectTemplate const* goInfo, TransportTemplate* transport)
{
    ...
    if (keyFrames[0].IsStopFrame())
    {
        ...
        keyFrames[0].DepartureTime = uint32(curPathTime * AsUnderlyingType(IN_MILLISECONDS));
    }
    ...
    for (size_t i = 1; i < keyFrames.size(); ++i)
    {
        ...
        if (keyFrames[i].IsStopFrame())
        {
            keyFrames[i].ArriveTime = uint32(curPathTime * AsUnderlyingType(IN_MILLISECONDS));
            keyFrames[i - 1].NextArriveTime = keyFrames[i].ArriveTime;
            curPathTime += float(keyFrames[i].Node->Delay);
            keyFrames[i].DepartureTime = uint32(curPathTime * AsUnderlyingType(IN_MILLISECONDS));
        }
        else
        {
