...
void WorldSession::SendPetBattleInitialUpdate(PetBattle* petBattle)
{
    ...
    for (uint8 i = 0; i < MAX_PET_BATTLE_TEAM; i++)
    {
        ...
        for (uint8 v = 0; v < petBattle->Teams[i]->TeamPetCount; v++)
        {
            ...
            petUpdate.JournalInfo.BattlePetGUID = petBattle->BattleType == PET_BATTLE_TYPE_PVE && i == PET_BATTLE_PVE_TEAM_ID ? ObjectGuid::Empty : pet->JournalID;
            ...
            petUpdate.JournalInfo.BattlePetDBFlags = pet->Flags & ~BATTLE_PET_FLAG_CAPTURED;
