...
void WildBattlePetMgr::Load()
{
    ...
    do
    {
        ...
        if (mapID == -1)
        {
            TC_LOG_ERROR("server.error", "WildBattlePetMgr::Load() no map id found for zone %u", zoneID);
            continue;
        }
...
void WildBattlePetMgr::Populate(WildPetPoolTemplate* wTemplate, WildBattlePetPool* pTemplate)
{
    ...
    if (sDB2Manager.HasBattlePetSpeciesFlag(wTemplate->Species, BATTLE_PET_SPECIES_FLAG_UNTAMEABLE))
        return;
...
void WildBattlePetMgr::Depopulate(WildBattlePetPool* pTemplate)
{
    ...
    {
        Unit* unit = ObjectAccessor::FindUnit(creature.first);
...
void WildBattlePetMgr::UnreplaceCreature(Creature* creature)
{
    ...
    /*Unit* unit = ObjectAccessor::GetCreatureOrPetOrVehicle(pTemplate->ReplacedRelation[creature->GetGUID()]);
    if (!unit || !unit->ToCreature())
        return
...
void WildBattlePetMgr::LeaveBattle(Creature* creature, bool /*p_Defeated*/)
{
    ...
    Unit* unit = ObjectAccessor::FindUnit(creature->replacementFromGUID);
    if (!unit || !unit->ToCreature())
        return;
