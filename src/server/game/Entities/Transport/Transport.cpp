...
float Transport::CalculateSegmentPos(float now)
{
    ...
    float timeSinceStop = frame.TimeFrom + (now - (1.0f/AsUnderlyingType(IN_MILLISECONDS)) * frame.DepartureTime);
    float timeUntilStop = frame.TimeTo - (now - (1.0f/AsUnderlyingType(IN_MILLISECONDS)) * frame.DepartureTime);
