...
auto BattlepayManager::ProductFilter(Product product) -> bool
{
    ...
    for (auto& itr : product.Items)
    {
        ...
        {
            TC_LOG_ERROR("network.BattlePay", "BattlePay : The item : %u exists in duplicate !", itr.ItemID);
            return false;
        }
        ...
        if (auto itemTemplate = sObjectMgr->GetItemTemplate(itr.ItemID))
        {
            ...
            {
                TC_LOG_ERROR("network.BattlePay", "BattlePay : The item : %u is not available to class : %u !", itemTemplate->GetAllowableClass(), player->getClassMask());
                return false;
            }
            ...
            if (itemTemplate->GetRequiredReputationFaction() && uint32(player->GetReputationRank(itemTemplate->GetRequiredReputationFaction())) < itemTemplate->GetRequiredReputationRank())
            {
                TC_LOG_ERROR("network.BattlePay", "BattlePay : The item %u require a reputation : %u !", itemTemplate->GetRequiredReputationFaction(), uint32(player->GetReputationRank(itemTemplate->GetRequiredReputationFaction())));
                return false;
            }
...
void BattlepayManager::SendPointsBalance()
{
    ...
    if (auto player = _session->GetPlayer())
    {
        ...
        //player->SendCustomMessage("Credits:", data1);
    }
}
...
void BattlepayManager::Update(uint32 diff)
{
    ...
    case DistributionStatus::BATTLE_PAY_DIST_STATUS_ADD_TO_PROCESS:
    {
        ...
        case CharacterBoost:
        {
            auto const& player = ObjectAccessor::GetObjectInWorld(data.TargetCharacter, static_cast<Player*>(nullptr));
            if (!player)
                break;
