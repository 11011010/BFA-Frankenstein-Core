...
void InstanceSaveManager::LoadResetTimes()
{
    ...
    for (auto& mapDifficultyPair : sDB2Manager.GetMapDifficulties())
    {
        ...
        for (auto& difficultyPair : mapDifficultyPair.second)
        {
            ...
            uint32 period = uint32(((mapDiff->GetRaidDuration() * sWorld->getRate(RATE_INSTANCE_RESET_TIME)) / AsUnderlyingType(DAY)) * AsUnderlyingType(DAY));
            if (period < DAY)
                period = DAY;
...
time_t InstanceSaveManager::GetSubsequentResetTime(uint32 mapid, Difficulty difficulty, time_t resetTime) const
{
    ...
    time_t period = uint32(((mapDiff->GetRaidDuration() * sWorld->getRate(RATE_INSTANCE_RESET_TIME)) / AsUnderlyingType(DAY)) * AsUnderlyingType(DAY));
    if (period < DAY)
        period = DAY;
